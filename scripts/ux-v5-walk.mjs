#!/usr/bin/env node
/**
 * UX Walk v5 — 2026-05-05
 * Fifth UX pass. Verifying Wave 17 shipped fixes:
 *   - jade badge fix (df43cb4e)
 *   - aeqi lowercase in identity templates (50255f64)
 *   - governance on-chain wiring (033ce7d7)
 *
 * Post-FORCE-UI-REDEPLOY. Live hash: index-DOFUk7ze.js
 *
 * Per-fix verification:
 *   1. Pill buttons — count action pill buttons, expect drop vs v4 (17/27 routes)
 *   2. Plan name — /c/<entity>/settings, check H2 isn't a UUID
 *   3. Jade badges — /c/<entity>/governance, check role-count badge color
 *   4. AEQI lowercase — agent pages, search for "AEQI" in user-facing copy
 *   5. Avatar — check no fuchsia (already passed v4)
 *
 * Output:
 *   Screenshots → /home/claudedev/aeqi/.observations/ux-v5/
 *   Raw JSON   → /home/claudedev/aeqi/.observations/ux-v5/raw.json
 *
 * Usage:
 *   AEQI_WEB_SECRET=... node scripts/ux-v5-walk.mjs
 */
...
const SCREENSHOT_DIR = join(REPO_ROOT, ".observations", "ux-v5");
...
const DETECT_ANTIPATTERNS = `() => {
  ...
  // 3. AEQI uppercase — v5: distinguish user-typed vs product copy
  //    AEQI in quest/idea titles is user-generated content; skip those
  //    We check for AEQI in structural copy elements (nav, sidebar, headings, placeholders)
  ...
  // 4. Pill buttons — ENHANCED: distinguish action vs sidebar/avatar
  ...
  for (const el of interactiveEls) {
    ...
    if (r > 8) {
      // Heuristic: if roughly square (w ≈ h and h <= 56), likely avatar/icon
      const isIconLike = Math.abs(w - h) < 12 && h <= 56 && text.length <= 3;
      // Or if radius >= 50% of height — true pill shape on text button
      const isPill = r >= h * 0.4 && !isIconLike;
      if (isPill) {
        actionPillCount++;
        if (actionPillExamples.length < 3) {
          actionPillExamples.push(r + "px \\"" + text + "\\"");
        }
      } else {
        avatarPillCount++;
      }
  ...
  if (avatarPillCount > 0) {
    issues.push({ code: "ICON_PILL_BUTTONS", severity: "info",
      detail: avatarPillCount + " icon/avatar circular elements (expected if circular design)" });
  }
  ...
  // 10. Personal rail leak (Ownership/Governance on /me/*)
  ...
  // 11. Jade/teal badges — v5: enhanced with exact rgb(46,143,113) check
  ...
  for (const el of badgeEls) {
    ...
    for (const colorProp of [s.color, s.backgroundColor]) {
      ...
      if (m) {
        const [, r, g, b] = m.map(Number);
        // Jade/teal: G significantly higher than R and B
        // Specifically catches rgb(46, 143, 113) from v4 findings
  ...
  // 12. Raw UUID visible as primary text (plan name regression)
  ...
  // 15. v5 SPECIFIC: Governance badge detailed color check
  //     Sample the exact computed color of elements near "role" count text
  ...
  // 16. v5 SPECIFIC: Plan name heading on settings
  if (window.location.pathname.includes("settings")) {
    const planSection = Array.from(document.querySelectorAll("h2,h3,[class*=plan],[class*=Plan]")).slice(0, 20);
    for (const el of planSection) {
      const t = (el.innerText || "").trim();
      if (/plan/i.test(el.className) || /plan/i.test(t)) {
        issues.push({ code: "SETTINGS_PLAN_LABEL", severity: "info",
          detail: 'Plan heading: "' + t.slice(0, 80) + '" tag=' + el.tagName + ' class=' + (el.className||"").slice(0,40) });
      }
    }
  }
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    if (result.consoleErrors.length > 0) {
      for (const e of result.consoleErrors.slice(0, 2)) {
