#!/usr/bin/env node
/**
 * UX Walk v7 — 2026-05-05
 * Seventh UX pass. Post Wave 20 fixes:
 *   - 481ffccc: Hairlines sweep — 261→124 1px borders removed system-wide
 *   - 78c2532f: Gov copy "See proposals (Phase 2)" gone + Director "Unoccupied" fallback
 *   - 49a2a89e + cc519c4: AEQI uppercase → lowercase in template seed ideas
 *   - 341fd156: Wallet Phase 2 — passkey upgrade section in Settings
 *   - 5fa98b02: Paymaster proof (backend, no UI)
 *
 * Live bundle: index-Gh8ui1LQ.js (Wave 20)
 *
 * Per-fix verification:
 *   1. Hairline count: should drop substantially from v6 (22/27 routes, 6-37/route)
 *   2. Gov copy: "See proposals (Phase 2)" must be GONE from company-governance
 *   3. Director "Unoccupied": company-roles Director row — no raw UUID as occupant
 *   4. AEQI uppercase: DB-stored existing records only, not new templates
 *   5. Wallet section: /me/settings Wallets panel shows "Upgrade to passkey" affordance
 *
 * v6 carry-forward:
 *   - AEQI_UPPERCASE_STRUCTURAL false positives on agent-overview/sessions (session-rail ASIDE)
 *   - P3: Personal treasury "Company" copy, "FOUNDERS· 1" spacing
 *
 * Output:
 *   Screenshots → /home/claudedev/aeqi/.observations/ux-v7/
 *   Raw JSON   → /home/claudedev/aeqi/.observations/ux-v7/raw.json
 *
 * Usage:
 *   AEQI_WEB_SECRET=... node scripts/ux-v7-walk.mjs
 */
...
const SCREENSHOT_DIR = join(REPO_ROOT, ".observations", "ux-v7");
...
console.log(`Bundle under test: index-Gh8ui1LQ.js (Wave 20)`);
...
async function visitRoute(context, route) {
  ...
  try {
    ...
    // ── v7 specific checks (separate evaluate calls to avoid template literal issues) ──
    try {
      const v7Issues = await page.evaluate(() => {
        const issues = [];
        const allText = document.body ? (document.body.innerText || "") : "";
        const path = window.location.pathname;

        // v7-A: Governance copy — "See proposals (Phase 2)" must be gone
        if (allText.includes("See proposals") && allText.includes("Phase 2")) {
          issues.push({ code: "BUILD_NOTE_EXPOSED", severity: "P2",
            detail: '"See proposals...Phase 2" internal note still present — Wave 20 fix not landed' });
        }
        if (path.includes("governance") && allText.includes("No proposals yet")) {
          issues.push({ code: "GOV_COPY_FIXED", severity: "info",
            detail: '"No proposals yet" copy confirmed — Wave 20 governance fix landed' });
        }
        if (path.includes("governance") && !allText.includes("Phase 2") && !allText.includes("No proposals yet")) {
          issues.push({ code: "GOV_COPY_UNKNOWN", severity: "info",
            detail: "Governance loaded but could not confirm new empty-state copy" });
        }

        // v7-B: Director UUID occupant — company-roles list (not detail page)
        if (path.includes("/roles") && !path.match(/\/roles\/[0-9a-f]/)) {
          const uuidRe = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/;
          // Find "Director" text element and check nearby text for UUID
          ...
          if (dirEl) {
            ...
            if (uuidRe.test(cardText)) {
              issues.push({ code: "DIRECTOR_UUID_OCCUPANT", severity: "P2",
                detail: 'Director card still shows raw UUID as occupant. Text: "' + cardText.trim().slice(0, 120) + '"' });
            } else if (/Unoccupied/i.test(cardText)) {
              issues.push({ code: "DIRECTOR_OCCUPANT_FIXED", severity: "info",
                detail: 'Director shows "Unoccupied" — Wave 20 fix confirmed' });
            } else {
              issues.push({ code: "DIRECTOR_OCCUPANT_INFO", severity: "info",
                detail: 'Director occupant text: "' + cardText.trim().slice(0, 100) + '"' });
            }
          }
        }

        // v7-C: Wallet upgrade section — /me/settings
        if (path.startsWith("/me") && path.includes("settings")) {
          const walletTexts = Array.from(document.querySelectorAll("*"))
            .filter(el => el.childNodes.length <= 5 && /upgrade.*passkey|passkey.*enrolled|upgrade wallet/i.test((el.innerText || "").trim()))
            .map(el => (el.innerText || "").trim().slice(0, 80));
          if (walletTexts.length > 0) {
            issues.push({ code: "WALLET_UPGRADE_SECTION", severity: "info",
              detail: 'Wallet upgrade section found: "' + walletTexts[0] + '"' });
          } else {
            const walletsHeader = Array.from(document.querySelectorAll("*")).some(el => {
              const t = (el.innerText || "").trim();
              return t === "Wallets" || t === "Wallet";
            });
            if (walletsHeader) {
              issues.push({ code: "WALLET_SECTION_NO_UPGRADE", severity: "P2",
                detail: "Wallets panel present but no passkey upgrade affordance — Wave 20 wallet section may not have rendered" });
            }
          }
        }

        // v7-D: AEQI uppercase in company-overview identity tile (DB-stored pre-fix)
          ...
          for (const el of missionEls) {
            if (/\bAEQI\b/.test(el.innerText || "")) {
              ...
              issues.push({ code: "MISSION_AEQI_UPPERCASE", severity: "P2",
                detail: 'Identity tile still has uppercase AEQI (DB-stored pre-fix record)' });
              break;
            }
          }
          if (!missionAeqi) {
        ...
        // v7-E: AEQI_UPPERCASE_STRUCTURAL refinement — exclude session-rail ASIDE
        // Re-run structural check with session-rail elements excluded
        // (The main detector in DETECT_ANTIPATTERNS still catches the old way;
        //  this checks if after exclusion the count drops — i.e., confirms FP rate)
        if (path.match(/\/agents\//) && (path.endsWith("/agents/" + path.split("/agents/")[1].split("/")[0]) || path.includes("/sessions"))) {
          ...
          if (sessionRailEl) {
            ...
            issues.push({ code: "SESSION_RAIL_AEQI_INFO", severity: "info",
              detail: "Session rail AEQI count: " + railAeqi + "x (these are user content FPs in structural detector)" });
          }
        }

        // v7-F: Personal treasury "Company" copy check
        if (path.startsWith("/me") && path.includes("treasury")) {
          if (allText.includes("This Company isn")) {
            issues.push({ code: "PERSONAL_TREASURY_COMPANY_COPY", severity: "P3",
              detail: 'Personal treasury still says "This Company..." — should say "account"' });
          }
        }

        return issues;
      });
      antiPatterns = antiPatterns.concat(v7Issues);
    } catch (e) {
      console.warn(`  v7 checks eval error on ${route.label}: ${e.message}`);
    }
