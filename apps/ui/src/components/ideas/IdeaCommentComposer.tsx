 ...
 *
 * The previous module-level `knownUnavailable` flag was a cross-instance
 * mutable gate: one `unknown_command` could disable every composer until
 * reload. The backend is shipped, so errors now flow through onError.
 */
...
export default function IdeaCommentComposer({
  ...
  const submit = useCallback(async () => {
    ...
    setSubmitting(false);
  }, [body, submitting, ideaId, user, onOptimistic, onConfirm, onError]);
