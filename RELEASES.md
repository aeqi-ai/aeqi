...
**Headline:** Inbox gate binds; /me/* dropped; account row simplified.

- **`AEQI_WEB_SECRET` env-var override patched across all 15 tenant systemd units** (`crates/aeqi-web/src/auth.rs` + ops sweep): the platform's authed proxy mints scope tokens with `AEQI_WEB_SECRET`, but per-tenant runtimes were validating against an ephemeral fallback key — the `x-aeqi-caller-user-id` participant gate added in v0.62.0 was being silently dropped on token-mismatch, so `/api/inbox` fell back to entity-scoped listing and the AEIQ tenant was leaking 27 cron `schedule:*` rows cross-tenant. Wired `AEQI_WEB_SECRET` to take precedence over the ephemeral fallback in the runtime's auth verifier; patched all 15 `aeqi-host-<entity_id>.service` units to inject the shared secret; restarted the fleet. Cross-tenant inbox leak: 27 → 0. The participant-gate now binds end-to-end. `8eecc164`.
- **`/me/*` entity-scope dropped — `/account` is the canonical personal surface** (`apps/ui/src/App.tsx` + `apps/ui/src/components/shell/AccountDropdown.tsx` + sibling routing): the parallel `/me/*` URL family was a mid-pivot artefact from when "personal entity" was being modelled as a Company sibling. Per `architecture_user_account_is_company.md` the user account IS a Company under the hood — there's exactly one personal entity per user and it's already addressable at `/c/<id>` / `/trust/<addr>`. The `/me/*` URLs duplicated affordance with `/account` (settings) and the personal Company's own primitive routes (agents/events/quests/ideas/treasury). Dropped the `/me/*` route family; `/account` replaces the settings surface; primitive lists for the personal Company resolve through the canonical entity URL like every other Company. Sidebar account row, header back affordances, and the few internal `/me/*` link generators all updated. `8aa6b9aa`.
- **Sidebar account row IS the menu trigger — chevron + "Personal Inbox" sub-item dropped** (`apps/ui/src/components/shell/AccountDropdown.tsx`): v0.58.0 split the account tile into two affordances — full-row `<Link to="/me">` + secondary `⋯` chevron Popover trigger. With `/me/*` retired in this ship, the dual-affordance shape no longer carries weight. Restored the canonical Popover-as-row-trigger pattern (per `8d0eaa45` flex recipe): the entire account row IS the menu trigger; chevron dropped; "Personal Inbox" sub-item dropped (the Inbox at `/` is canonical for the user's identity). Sub-actions reduce to account · billing · sign-out. `3eaa82e0`.
- **Agent header `+ New` button matches the canonical `+New` shape** (`apps/ui/src/components/AgentSurfaceHeader.tsx`): drilled-agent header's primary action was rendering as a generic primary button rather than the canonical `+New` shape used on every primitive surface (Ideas / Quests / Events / Roles toolbars). Affordance identity wins — same action means same shape everywhere. Matched to the canonical IconButton-with-`+`-glyph + label shape. `3dc2ec47`.
